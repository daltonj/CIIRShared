...
public class GalagoIndexSearcher implements SearcherI {
	...
	throws Exception {
        ...
        Node transformed = m_retrieval.transformQuery(root);

		boolean useStemmming = CiirProperties.getPropertyAsBoolean("Searcher.performStemming", false);
		if (useStemmming) {
			queryParams.set("part", "stemmedPostings");
		}
		...
		for (int i = 0; i < galagoResults.length; i++) {
			...
			results.add(new Result(galagoResults[i].documentName, score));
			if (debug) {
				System.out.format("%s Q0 %s %d %s galago\n", query.getQueryNum(), galagoResults[i].documentName, rank,
						formatScore(score));
			}
		}
